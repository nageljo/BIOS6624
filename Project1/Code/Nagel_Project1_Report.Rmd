...
(
    ...
    %>% select(newid, timepoint, agg_ment, agg_phys, leu3n,
               vload, hard_drugs, age, bmi, smoke, educbas, race)
    %>% pivot_wider(
        id_cols = newid,
    ...
    %>% mutate(
        ...
        log_ratio_vload = log10(vload_2/vload_baseline)
    )
    %>% select(-matches('_2'))
) -> hiv
```
...
# frequentist models
agg_ment_mod_freq <- lm(get_formula("log_ratio_vload"), data = hiv)
summary(agg_ment_mod_freq)
```

```{r}
anova(agg_ment_mod_freq)
```
