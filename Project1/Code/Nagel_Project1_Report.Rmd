...
(
    ...
    %>% mutate(
        ...
        smoke = recode_values(
            ...
            NA ~ NA_character_,
            default = "Not Handled"
        ) %>% factor(levels = c("Never Smoked", "Former Smoker", "Current Smoker")),
...
```{r}
# fit Bayesiam models
bmod_ment <- fit_brm(formula = formula_ment, data = hiv_clean)
bmod_phys <- fit_brm(formula = formula_phys, data = hiv_clean)
bmod_vload <- fit_brm(formula = formula_vload, data = hiv_clean)
bmod_leu3n <- fit_brm(formula = formula_leu3n, data = hiv_clean)
```
```{r}
summary(bmod_leu3n)
```
