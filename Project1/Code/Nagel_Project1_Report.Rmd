...
(
    ...
    %>% select(newid, timepoint, agg_ment, agg_phys, leu3n,
               log_vload, hard_drugs, adh, age, bmi, smoke_recode, edu_recode, race_recode)
    %>% pivot_wider(
        id_cols = newid,
...
get_formula <- \(outcome) paste0(outcome, " ~
                            hard_drugs_baseline + age_baseline + bmi_baseline + smoke_recode_baseline + edu_recode_baseline + race_recode_baseline")
