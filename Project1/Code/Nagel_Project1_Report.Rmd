...
(
    ...
    %>% select(newid, timepoint, agg_ment, agg_phys, leu3n,
               log_vload, hard_drugs, adh, age, bmi, smoke, edu_recode, race_recode)
    %>% pivot_wider(
        id_cols = newid,
...
table1 <- (
    ...
    %>% tbl_summary(
        ...
        label = list(                              
            agg_ment_baseline   ~ "SF-36 Mental QoL",
            agg_phys_baseline   ~ "SF_36 Physical QoL",
            leu3n_baseline      ~ "CD4 Count",
            log_vload_baseline  ~ "Log Viral Load",
            adh                 ~ "Adherence",
            bmi_baseline        ~ "BMI",
            smoke_baseline      ~ "Smoking",
            edu_recode_baseline ~ "Education",
            race_recode_baseline~ "Race/Ethnicity"
        ),
        include = -c(matches("diff"), newid)
    )
