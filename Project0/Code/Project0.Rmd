...
options(kable_styling_latex_options = "hold_position")
```
...
The linear mixed-effect model comparing the booklet sample times to the MEMS cap sample times (**Table 2**) revealed significant attative bias between the measurements. The model's slope was not significantly different from 1 (p-value = 0.2) indicating no proportional bias (bias did not change with time) between the measurement  methods. However, the intercept was significantly different from 0 (p-value < 0.001) indicating addative bias. At waking, MEMS cap sample times were on average 10.2 minutes (95% CI: 4.4, 15.9) minutes later than booklet sample times. The relationship is visualized in **Figure 1**. While the model showed slight additive bias between the measurements, the plot shows high agreement between the two methods, with the fitted and perfect agreement ($y=x$) lines almost completely overlapping.
