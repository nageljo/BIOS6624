...
Create scatterplot of captime since wake vs booklet time since wake (DO THIS)


book_min_from_wake = book time - diary wake time
LMM: book_min_from_wake ~ cap_time_since_wake + (1 | id)
    intercept: 0 for perfect agreement
    slope: 1 for perfect agreement
    if these are not 0 and 1 i need to interpret this
    what do we expect if there is perfect agreement and 
...
what proportion of samples are within 7.5 minute window for each timepoint separately?

what proportion of samples are within 15 minute window for each timepoint separately?
...
piecewise with a break at 30 mins
